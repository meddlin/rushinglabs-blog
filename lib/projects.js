...
export async function getProjectPage(project, id) {
    ...
    const fullPath = path.join(projectsDirectory, `${project}/${id}.mdx`);
...
/**
 * Get a list of file names in the '/projects' directory
 * (excluding '.md' extension from name)
 * @returns 
 */
export function getAllProjectIds() {
    let fileNames = getAllFiles(projectsDirectory);
    var postInfo = [];
    ...
    for(var i = 0; i < fileNames.length; i++) {
        ...
        let project = splitPath[splitPath.length - 2];
