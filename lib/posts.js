...
export async function getPostData(year, id) {
    debugger;
    const fullPath = path.join(postsDirectory, `${year}/${id}.mdx`); // TODO: Support .md files
    ...
    console.log(`getPostData - frontMatter: ${frontMatter}`)
    console.log(`getPostData - frontMatter: ${frontMatter.data.date}`)
    // console.log(`getPostData - frontMatter: ${JSON.stringify(frontMatter)}`)

    // console.log(`getPostData - frontMatter.content: ${frontMatter.content}`) // frontMatter.content is the MD post
...
export function getAllPostIds() {
    ...
    for(var i = 0; i < fileNames.length; i++) {
        ...
        //     noExt = fileNames[i].replace(/\.md$/, '');
        // else 

        if (fileNames[i].endsWith('.mdx'))
            noExt = fileNames[i].replace(/\.mdx$/, '');
        
        // console.log(`getAllPostIds - noExt: ${noExt}`);

        let splitPath = noExt.split(path.sep);
        

        let fileName = splitPath[splitPath.length - 1];
        // console.log(`getAllPostIds - fileName: ${fileName}`);
        let year = splitPath[splitPath.length - 2];
        // console.log(`getAllPostIds - year: ${year}`);
...
export function getSortedPostsData() {
    ...
    console.log(`getSortedPostsData - postsDirectory: ${postsDirectory}`);
    console.log(`getSortedPostsData - fileNames: ${fileNames}`);
 ...
 * MDX handling
 */
// export function MDX_fetchPostIds() {
//     /* return fs.promises.readdir(path.join(process.cwd(), "posts/2021")); */
