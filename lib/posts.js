...
export function getCategoryPosts(category) {
    ...
    // console.log('getCategoryPosts() -- `allCategoryPosts`');
...
export async function getSortedPostsData() {
    ...
    const newData = await Promise.all(allPostsData.map(async (post) => {
        const postContent = await getPostData(post.date.slice(0, 4), post.id.slice(5));
