...
export function getAllDocsSections() {
    ...
    // Mocked an array of string(s) for now.
    return ['owasp'];
}
...
export function getPostsFromDocsSubdir(directory) {
    ...
    console.log(`getPostsFromDocsSubdir - ${postsArr}`)
    let docsSubdirPosts = [];
...
///
// Retrieve a list of file names in the `/docs` directory (excluding '.md' extension from name) 
///
export function getAllDocsIds() {
    let fileNames = getAllFiles(docsDirectory);
    console.log(`getAllDocsIds - ${fileNames}`)
    var postInfo = [];
    ...
    for(var i = 0; i < fileNames.length; i++) {
        ...
        console.log(`getAllDocsIds [i] - ${fileNames[i]}`)

        let splitPath = noExt.split(path.sep);
        let fileName = splitPath[splitPath.length - 1];
        let section = splitPath[splitPath.length - 2];

        console.log(`getAllDocsIds - ${fileName}, ${section}`)
