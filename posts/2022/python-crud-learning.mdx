...
```python
res = db.query(Budget).join(Category).filter(Budget.id == Category.budget_id) #.all()
    print(res)
    return res
```
...
```python
res = db.query(Budget.name, Category.name).join(Category).filter(Budget.id == Category.budget_id) #.all()
    print(res)
    return res
```
...
custom name for each column. Without this, the `key` is the same as the column name setup in 
...
However, an `.all()` query is _a series of rows_.
